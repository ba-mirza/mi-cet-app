...
const fetchCurrencies = async (): Promise<Currency[]> => {
  try {
    const response = await fetch(
      `https://api.currencyapi.com/v3/latest?apikey=cur_live_1ohFR6u0zRejdmWaA4qeAHwUcVvod2NCA3c97ZQZ`
    );
...
export default function CurrencyConverter() {
  ...
  return (
    <div className="flex border border-[#34343450] flex-col justify-center w-full max-w-[976px] lg:w-[976px] h-auto lg:h-[217px] px-4 lg:px-[90px] py-4 lg:py-0 rounded-lg bg-white mt-8 mx-4 lg:mx-0">
      <div className="flex flex-col lg:flex-row items-center justify-center gap-2 lg:gap-0">
        {/* Поле ввода суммы и валюта FROM */}
        <div className="flex border border-gray-300 rounded-lg w-full lg:w-[400px] p-2 mx-0 lg:m-2 focus-within:border-blue-500">
          <input
            type="text"
            value={amount}
            placeholder="0"
            className="w-full px-4 text-lg lg:text-[24px] font-medium outline-none"
            onChange={handleAmountChange}
          />
          <select
            value={fromCode}
            className="mr-2 border-none focus:border-none focus:outline-none focus:ring-0 focus:shadow-none text-sm lg:text-[16px] font-medium bg-transparent"
        ...
        <button
          ...
          className="mx-2 px-8 py-2 border border-gray-300 rounded-lg lg:border-none hover:bg-gray-100 transition-colors order-last lg:order-none"
        ...
        <div className="flex border border-gray-300 rounded-lg w-full lg:w-[400px] p-2 mx-0 lg:m-2">
          <input
            type="text"
            value={convertedAmount}
            disabled
            className="w-full px-4 text-lg lg:text-[24px] font-medium outline-none bg-gray-50 text-gray-700"
          />
          <select
            value={toCode}
            className="mr-2 border-none focus:border-none focus:outline-none focus:ring-0 focus:shadow-none text-sm lg:text-[16px] font-medium bg-transparent"
      ...
      <div className="p-2 lg:p-[8px] text-center mt-2">
        <h1 className="font-medium text-xl lg:text-4xl mb-1 break-words">
          {amount || "0"} {fromCode} ={" "}
          <span className="text-blue-600">{convertedAmount}</span> {toCode}
        </h1>
        <span className="font-medium text-xs lg:text-sm text-gray-600">
          Средний рыночный курс обмена на{" "}
